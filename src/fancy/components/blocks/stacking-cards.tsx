...
export default function StackingCardsDemo() {
  ...
  const [lenisWrapper, setLenisWrapper] = useState<HTMLElement | null>(null)
  ...
  useEffect(() => {
    if (lenisRef.current) {
      setLenisWrapper(lenisRef?.current?.wrapper || document.documentElement)
    }
  }, [])

  const { scrollYProgress } = useScroll({
    container: lenisRef.current
      ? {
          current: lenisWrapper,
        }
      : undefined,
  ...
  return (
    <ReactLenis ref={lenisRef} className="w-full h-full overflow-auto bg-white">
      <div ref={wrapperRef}>
