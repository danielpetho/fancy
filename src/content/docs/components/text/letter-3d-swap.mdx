...
This creates the illusion of characters flipping between two faces of a 3D cube. The interactive demo below shows how these transforms work together:

<ExplanationDemo name='letter-3d-swap-explanation-top-demo' />

#### Left and right rotations

For left/right rotations, we use a bit more complex series of transforms to construct a correctly sized box. For this, we cannot bring the first face by `0.5lh` since the other side of the box shouldn't be that size, rather it should be equal of the width of the character. Now, we cannot know the exact width of the character in advance, as each individual character can have a different width (except monospace fonts where all characters are the same width). We can still use percentage-based translations, at least on the X and Y axes (not on Z though). So, to bring the first face by the correct transform, first, we have to rotate it 90° (or -90°) on the Y axis, translate it by hald width (translateX(50%)), then rotate it back by 90° (or -90°) on the Y axis. Then we do the transform for the second face. Check out the demo for the chain:

<ExplanationDemo name='letter-3d-swap-explanation-left-demo' />

Of course, you can achieve the same result by applying (other) transforms in a different order, and I apologise if it seems overcomplicated. This is how it made sense to me :).
