...
The trick here is similar to the [Letter Swap Hover](/docs/components/text/letter-swap-hover) component—duplicate the text, then wrapping the them in a container with `relative` position, then stack the elements vertically. We use `useScroll` hook from framer-motion to track the scroll position of the container, and use the `scrollYProgress` value to offset the vertical position of the elements (by setting the `y` property of the element).

## Notes

In order to achieve a nice effect, you likely have to play with the container (where to track the scroll) and its offset. Please refer to framer's [documentation](https://www.framer.com/motion/use-scroll/) for more details.
