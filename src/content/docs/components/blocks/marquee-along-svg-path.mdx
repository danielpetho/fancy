...
The component is really similar to the [Simple Marquee Component](https://fancy.component-driven.dev/docs/components/blocks/simple-marquee), and has the same features and props (and a bit more:)):
...
Before you dive into understanding this component, please read through the [Simple Marquee](https://fancy.component-driven.dev/docs/components/blocks/simple-marquee) component's documentation, as this one is almost identical.
...
Most of these factors are calculated inside an `useAnimationFrame` hook, which runs every frame. Most of these values are either motion values or refs to avoid unnecessary re-renders. Please refer to the Simple Marquee Component [documentation](https://github.com/danielpetho/fancy/blob/main/src/fancy/components/blocks/marquee-along-svg-path.tsx), there is a detailed explanation for each part.
...
Keep an eye on performace. The more complex and longer the SVG path, and more elements you have, the more performance issues you might encounter.

## Resources

- [Simple Marquee Component](https://fancy.component-driven.dev/docs/components/blocks/simple-marquee)
