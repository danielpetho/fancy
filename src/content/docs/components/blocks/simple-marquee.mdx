...
At the heart of the component is the `useAnimationFrame` hook from Motion, which executes our anim code on every frame. Here's how it works:
...
3. We use `useTransform` to convert these values to percentages and apply wrapping:
...
4. On each animation frame, we increment the position values:
...
This example demonstrates the basic wrapping mechanism:
...
The component solves this by using the `repeat` prop to duplicate all child elements multiple times:
...
By default, the `repeat` value is 3, which means your content is duplicated three times. With enough repetitions, new elements enter the visible area before existing ones leave, creating a seamless continuous animation. Try increasing the `repeat` value in the demo above to see how it eliminates the jumpiness.
...
- `smoothHoverFactor` uses spring physics for smooth transitions between speeds
- `slowDownSpringConfig` lets you customize the spring animation parameters

### Scroll-Based Velocity

When `useScrollVelocity` is enabled, the component tracks scroll velocity and uses it to influence the marquee:
...
- Spring physics ensure smooth transitions as scroll velocity changes
...
The easing function receives a normalized value between 0 and 1 and should return a transformed value. This can create effects like:

- Accelerating/decelerating movement
- Bounce effects at certain points
- Any custom motion pattern
...
The drag implementation uses pointer events for direct user control:
...
During animation frames, dragging takes precedence over other movement factors:
...
When the user stops dragging, velocity gradually decays:
...
A powerful feature of this component is its ability to work with CSS 3D transforms. You can apply perspective and rotation to the marquee container or its children:
...
>
  <SimpleMarquee dragAngle={45}>
    <div style={{ transform: "rotateY(20deg)" }}>{/* Content */}</div>
  </SimpleMarquee>
</div>
```

For angled marquees, the `dragAngle` prop becomes particularly useful:
...
This ensures that dragging works intuitively even when the marquee is rotated or transformed in 3D space.
