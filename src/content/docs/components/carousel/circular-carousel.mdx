...
When `continuousFocus` is enabled, items will continuously interpolate properties defined in `focusStyleInterpolation` based on their angular distance from the `focusOrigin`. The focus mapping is limited to only the next and previous elements relative to the focus origin, creating a smooth transition between adjacent items.
...
- **Limited range**: Focus effects only apply between the next and previous elements relative to the focus origin
...
- **Unified interpolation**: Use `focusStyleInterpolation` to animate both CSS variables and motion properties with from/to ranges using `useTransform`
