...
There must be the same number of values in a `times` array as there are keyframes for that property. If `times` is omitted, the keyframes are spread evenly across the `duration`. You can read more about this in the [Motion `animate` function documentation](https://motion.dev/docs/animate#times).
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>`{ x?: DynamicAnimationOptions, y?: DynamicAnimationOptions }`</TableCell>
