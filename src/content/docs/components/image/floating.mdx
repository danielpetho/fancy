...
The `Floating` component creates an interactive floating effect through several key mechanisms:
...
For example, if you have 6 floating images, instead of creating 6 separate `FloatingElement` components, you could group them into 3 pairs. This reduces the animation calculations from 6 to 3, improving performance while maintaining a similar visual effect.

### Directional Control
The `depth` and `sensitivity` props offer flexible control over movement direction:
