...
<ComponentPreview name="gooey-filter-demo" />
...
<CodeBlockWrapper language="bash">{`npx shadcn@latest add "https://fancycomponents.dev/r/gooey-filter.json"`}</CodeBlockWrapper>
...
<ComponentSource name="gooey-filter" />
...
Add the `GooeyFilter` component to your project, pass an `id` prop to the component (optional), then use the same `id` prop in the `filter` CSS property of the container you want to apply the filter to. High-level example:

<CodeBlockWrapper language="typescript" className="mt-4">
  {`<GooeyFilter id="gooey-filter" />
...
<ComponentPreview name="gooey-filter-menu-demo" />

### Rounded corners

The following example combines the [PixelTrail](/docs/components/background/pixel-trail) component with the GooeyFilter component to create a rounded-at-all-corners effect. Unfortunately, the component doesn't support Safari, so you'll need to create a fallback for that.

<ComponentPreview name="gooey-filter-pixel-trail-demo" />
