...
<CodeBlockWrapper language="bash">{`npx shadcn@latest add "https://fancycomponents.dev/r/gravity.json"`}</CodeBlockWrapper>
...
<Step>First, install the following dependencies.</Step>{" "}
...
<CodeBlockWrapper
  className="mt-2"
  language="bash"
>{`npm install matter-js @types/matter-js poly-decomp svg-path-commander`}</CodeBlockWrapper>
